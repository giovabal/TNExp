...
| `FETCH_RECOMMENDED_CHANNELS` | If `True`, `get_channels` fetches Telegram-recommended channels for each interesting channel after crawling and adds any new ones to the database. New channels are not crawled automatically; mark them as interesting to include them in the next run. | `False` |
