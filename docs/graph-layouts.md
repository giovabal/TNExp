...
Pulpit always computes **ForceAtlas2** as its primary layout. Additional layouts can be pre-computed at export time with `--extra-layouts` (2D) and `--extra-layouts-3d` (3D). Once computed, layouts are selectable from a dropdown in the graph viewer with smooth animated transitions — no re-export required.
