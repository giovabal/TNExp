...
# neither marked in-target nor referenced (forwarded-from) by in-target channels.
#
# When to use:
#   - After un-marking organisations/channels as in target, to reclaim disk space.
...
in_target_ids = set(Channel.objects.in_target().values_list("id", flat=True))
referenced_ids = set(
...
keep_ids = in_target_ids | referenced_ids
