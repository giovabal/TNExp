...
- **Consensus matrix zoom/pan**: the matrix is rendered inside a fixed-height viewport. Mouse-wheel zooms centered on the cursor; click-drag pans; pinch-to-zoom on touch. `+` / `−` / **Fit** buttons and a zoom-% readout are shown above the matrix. Keyboard shortcuts: `0` = fit to window, `+`/`=` = zoom in, `−` = zoom out.
