...
*Competition vs. cohesion metrics. Custom backoffice admin.*
...
- **Backoffice admin** (`/manage/`) — new staff-only app replacing Django admin for day-to-day data management. Powered by a Django REST Framework JSON API (`/manage/api/`). Five sections: **Channels** (searchable/filterable table with inline org assignment, group chip management, and bulk assign/group operations for hundreds of channels at once), **Organizations** (inline CRUD with color picker and is_interesting toggle), **Groups** (inline CRUD), **Search Terms** (add-by-Enter list), **Events** (event types + events with date and type filters). A *Manage* button appears in the top navigation for staff users.
