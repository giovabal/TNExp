...
- **Timeline sparklines in `network_table.html`**: when a year-by-year timeline has been exported, each row in the Network Statistics table shows a mini bar chart between the metric label and its value. The first bar represents the full-range ("All") value in blue; subsequent bars show each year's value in gray. The active view's bar is highlighted in solid blue. Hovering a bar shows the exact value. A year-selector nav at the top of the page links directly to each per-year network table.
- **Year navigator in `channel_table.html` and `community_table.html`**: when a timeline is present, both pages now display the same pill-button year navigator as the Network Statistics page — **All** links to the full-range table, each year button links to the matching per-year table, and the active view is highlighted.
