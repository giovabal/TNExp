...
*Default configuration reorganized. Database maintenance.*

### New features
- **Interesting override on channels** — each `Channel` gains an `interesting_override` nullable boolean field (default `null`). When `null`, the channel's interesting status is determined by its organization as before. When `True`, the channel is always treated as interesting regardless of its organization. When `False`, the channel is always excluded regardless of its organization. Settable from the channel edit page and inline from the channel list (new **Override** column with a compact auto/yes/no select). The channel list also gains two new status filter options: *Forced interesting* and *Forced not interesting*. The `Channel.objects.interesting()` manager and the `Interesting` status filter in the backoffice API both respect the override.
...
- **Reactions per month chart on the home page** — a new collapsible panel mirrors the per-channel *Reactions per month* chart at the project level, aggregating reaction counts across all monitored channels and broken down by the eight most-used emojis. Each channel's `uninteresting_after` cutoff is respected. The chart shares its renderer with the channel-detail version (moved into the base template) so both stay in sync.
...
- **Channel count mismatch between crawler and homepage** — `Channel.objects.interesting()` (used by the homepage, channel list, and all views) excluded `is_private` channels but not `is_lost` channels, while the crawler excludes both by default. Channels marked as lost were therefore counted in the UI but silently skipped at crawl time. The manager now also excludes `is_lost=True`, making the displayed count consistent with the crawl scope.
