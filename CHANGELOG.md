...
- `WALKTRAP`: **Walktrap** (Pons & Latapy 2006). Computes short random-walk distances between channels (4 steps) and applies Ward's agglomerative clustering, producing a full dendrogram cut at the modularity-maximising partition. Symmetrises to undirected. No new dependency (uses existing `python-igraph`).
...
- **Consensus matrix zoom/pan**: the matrix is now rendered inside a fixed-height viewport. Mouse-wheel zooms centered on the cursor; click-drag pans; pinch-to-zoom on touch. `+` / `−` / **Fit** buttons and a zoom-% readout are shown above the matrix. Keyboard shortcuts: `0` = fit to window, `+`/`=` = zoom in, `−` = zoom out.

### Fixes
- **Compare networks — broken Consensus matrix link in Community Statistics**: `community_table.js` injected the "Consensus matrix" nav button with a hardcoded `href="consensus_matrix.html"`. Since this link is written by JavaScript (not in the HTML file), the `_patch_compare_html` string-replace pass never reached it, so `community_table_2.html` pointed to the wrong page. Fixed by deriving the page suffix at runtime from `window.DATA_DIR`.
