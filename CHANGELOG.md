...
### New features
- **Interesting override on channels** — each `Channel` gains an `interesting_override` nullable boolean field (default `null`). When `null`, the channel's interesting status is determined by its organisation as before. When `True`, the channel is always treated as interesting regardless of its organisation. When `False`, the channel is always excluded regardless of its organisation. Settable from the channel edit page and inline from the channel list (new **Override** column with a compact auto/yes/no select). The channel list also gains two new status filter options: *Forced interesting* and *Forced not interesting*. The `Channel.objects.interesting()` manager and the `Interesting` status filter in the backoffice API both respect the override.
