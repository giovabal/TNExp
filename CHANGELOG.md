...
- **Documentation coherence pass** — `docs/network-measures.md` gains a full `Diffusion lag` section (Kwon 2013; Cheng 2014) and the per-channel measure count corrects to 18 across README, CLAUDE.md, and the measures doc. `CONFIGURATION.md` is restructured into three sections matching the new `.env` / `.analysis-defaults` / `.system-options` split, with every key from `analysis-defaults.example` documented. `docs/web-interface.md` adds a Home page section (summary cards, time-series charts, project-level Reactions per month, recent-messages feed), a Maintenance subsection with the engine-specific strategy tables, a Graph viewer features section covering the info bar, and Lost messages / Retry lost messages / export-overwrite confirmation bullets on the channel detail and Operations panels. `docs/workflow.md` gains a `--diffusion-window` CLI example. CLAUDE.md's two stale `WORKFLOW.md` references and the CHANGELOG footer's references to non-existent `INSTALLATION.md` / `ANALYSIS.md` / `SCREENSHOTS.md` are fixed. `ROADMAP.md` drops the already-shipped "persist analysis options" item and a stray footer line; `PROPOSALS.md` is renumbered as a flat 1–6 list.
